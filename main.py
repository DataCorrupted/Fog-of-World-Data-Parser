...
def generate_image(tile, tmp_folder):
    ...
    size = FogMap.TILE_WIDTH * FogMap.BITMAP_WIDTH // ZOOM_LEVEL
    im = Image.new("RGBA", (size, size), (255, 255, 255, 0))
    for block in tile.blocks.values():
        for i in range(FogMap.BITMAP_WIDTH):
            for j in range(FogMap.BITMAP_WIDTH):
                x = block.x * FogMap.BITMAP_WIDTH + i
                y = block.y * FogMap.BITMAP_WIDTH + j
                x //= ZOOM_LEVEL
                y //= ZOOM_LEVEL
